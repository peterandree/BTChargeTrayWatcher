...
> This plan describes a design that shipped under different class names than proposed here (actual: `NtfyIntegrationSettings`, `NtfyNotificationChannel`, `NtfyTopicGenerator`, integrated into `OptionsForm.cs` — not `NtfySettings`/`NtfyNotificationService`/`NtfySetupWizard` as drafted). Preserved for historical reference only; do not use as an implementation reference.
